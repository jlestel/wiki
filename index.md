---
type: index
updated: 2026-04-17
---

# Wiki Index

## Sources

- [[2015-08-31-steve-jobs-11-phrases]] — 11 citations de Steve Jobs sur le travail (persévérance, artisanat, memento mori)
- [[2026-03-30-opencve-veille-vulnerabilites]] — OpenCVE : plateforme open source de veille CVE filtrée
- [[2026-04-01-openai-levee-122-milliards]] — OpenAI lève 122 Md$, valorisation 852 Md$, ouverture aux particuliers, projet super-app
- [[2026-04-03-espoir-avenir-radieux]] — Manifeste techno-optimiste de Brivael (Argil/YC) : IA, entrepreneuriat, anti-décroissance
- [[2026-04-12-france-linux-opensource]] — La France sort de Microsoft pour Linux/open source (souveraineté numérique)
- [[2026-04-14-apprendre-a-coder]] — L'IA est un levier, pas un remplacement : maîtriser data structures, algorithmes, systèmes
- [[2026-04-14-majorite-emmerde-llm]] — Critique du biais de majorité des LLM, sycophantie
- [[2026-04-15-big-tech-delegation-risque-ia]] — Les big tech délèguent le risque IA aux labs et au capital-risque pour récupérer la techno mature
- [[2026-04-15-comptes-ia-twitter]] — 15 comptes Twitter IA à suivre, cartographie de l'écosystème
- [[2026-04-15-coolify-alternative-vercel]] — Coolify : alternative open source self-hosted à Vercel/Netlify
- [[2026-04-15-google-apps-desktop]] — Google Search pour Windows + Gemini natif macOS
- [[2026-04-15-liens-tech-avril-2026]] — Sélection de self-hosted : Alexandrie, Dockman, Planka, Zerobyte
- [[2026-04-15-screenshot-claude-conversation]] — Dave Rupert : l'asymétrie d'effort pensée vs LLM
- [[2026-04-15-telegram-bot-go-lambda-zero]] — Bot Telegram en Go + AWS Lambda à coût zéro
- [[2026-04-15-terraform-gcp-scaling]] — Retour d'expérience Terraform GCP à l'échelle
- [[2026-04-16-claude-code-routines]] — Claude Code Routines : cron cloud d'agents Claude
- [[2026-04-15-tesla-fsd-hw3-refunds]] — Tesla FSD approuvé en Europe mais HW3 exclu : action collective de 1 400+ propriétaires
- [[2026-04-16-saying-goodbye-to-agile]] — Lewis Campbell enterre Agile : retour au spec-driven via Royce 1970 et les LLM
- [[2026-04-16-edgee-token-compression]] — Edgee : compression tokens LLM pour réduire les coûts
- [[2026-04-14-cybersecurity-proof-of-work]] — La cybersécurité comme proof-of-work : course aux tokens, Mythos (Anthropic), open source renforcé

## Entities

- [[anssi]] — Agence française de cybersécurité (porte CERT-FR)
- [[anthropic]] — Lab IA (Claude), 12 Md$ d'Amazon
- [[apple]] — Géant tech, stratégie IA zéro risque via distribution
- [[argil]] — Startup YC, génération vidéo IA, 40+ pays
- [[ark-invest]] — Gestionnaire ETF innovation
- [[amazon]] — Géant tech, investisseur OpenAI et Anthropic
- [[brivael]] — Cofondateur/CTO d'Argil, autodidacte, techno-optimiste
- [[chatgpt]] — Chatbot IA d'OpenAI (900M utilisateurs hebdo)
- [[claude-code]] — CLI agent Claude pour devs
- [[codex-openai]] — Outil de programmation IA d'OpenAI
- [[coolify]] — Plateforme self-hosted open source (alternative Vercel)
- [[dinum]] — Direction interministérielle du numérique (France)
- [[disney]] — Conglomérat divertissement, ex-partenaire Sora
- [[drew-breunig]] — Analyste tech, cybersécurité comme proof-of-work
- [[edgee]] — Startup compression tokens LLM
- [[gemini]] — Famille de modèles IA de Google (app macOS)
- [[girard]] — Anthropologue, théoricien du désir mimétique
- [[google]] — Géant tech, inventeur Transformer, seul à construire en interne
- [[hayek]] — Économiste, connaissance dispersée, marché comme traitement d'information
- [[lewis-campbell]] — Dev/consultant, critique d'Agile, avocat du spec-driven
- [[meta]] — Géant tech, Llama (open source)
- [[microsoft]] — Géant tech, 13 Md$ dans OpenAI (put option)
- [[mistral]] — Lab IA français
- [[nietzsche]] — Philosophe, création de valeurs propres
- [[nvidia]] — Fabricant GPU, investisseur OpenAI
- [[ollama]] — Runner LLM local, pillier IA souveraine
- [[openai]] — Entreprise IA, créatrice de ChatGPT
- [[softbank]] — Conglomérat japonais, investisseur OpenAI
- [[sora]] — Générateur vidéo IA d'OpenAI (fermé)
- [[steve-jobs]] — Cofondateur Apple, philosophie du travail (artisanat, design fonctionnel)
- [[taleb]] — Essayiste, antifragilité
- [[winston-royce]] — Ingénieur logiciel (1970), souvent mal cité comme père de Waterfall alors qu'il le disqualifiait
- [[elon-musk]] — CEO Tesla, admissions publiques sur la dette FSD/HW3
- [[mischa-sigtermans]] — Propriétaire Tesla NL, initiateur de l'action collective hw3claim.nl
- [[tesla]] — Constructeur automobile, controverse FSD/HW3 en Europe
- [[y-combinator]] — Accélérateur de startups

## Concepts

- [[antifragilite]] — Systèmes qui tirent profit du désordre (Taleb)
- [[asymetrie-pensee-ia]] — Asymétrie d'effort pensée humaine vs génération LLM
- [[automatisation-ia]] — Agents Claude en cron cloud : automatisation asynchrone
- [[biais-majorite-llm]] — LLM alignés sur la moyenne statistique, lissent les opinions minoritaires
- [[commoditisation-modeles-ia]] — Les LLMs n'ont pas de moat, rattrapés en quelques mois
- [[couche-applicative-ia]] — La vraie opportunité : applications par-dessus les modèles commoditisés
- [[critique-agile]] — Agile défini négativement, antériorité Royce 1970 ignorée
- [[decroissance]] — Réduction de la production comme réponse écologique (critiquée)
- [[delegation-risque-big-tech]] — Big tech délèguent le risque R&D aux labs et au VC
- [[democratisation-ia]] — L'IA réduit les asymétries de capacité entre individus
- [[design-fonctionnel]] — Design = comment un objet fonctionne, pas à quoi il ressemble (Jobs)
- [[droit-consommateurs-tech]] — Protection juridique consommateurs face aux promesses logicielles non tenues
- [[desir-mimetique]] — On désire ce que les autres désirent (Girard)
- [[entrepreneuriat-france]] — Diagnostic de l'environnement entrepreneurial français
- [[excellence-artisanale]] — Qualité intégrale, même sur ce qui ne se voit pas (Jobs : métaphore du charpentier)
- [[fondamentaux-cs]] — 3 piliers : data structures, algorithmes, systèmes
- [[ia-comme-levier]] — L'IA amplifie une compétence existante, ne la remplace pas
- [[infrastructure-as-code]] — IaC, Terraform, bonnes pratiques à l'échelle
- [[levee-de-fonds-ia]] — Montants records levés par les entreprises IA
- [[memento-mori]] — Conscience de la mort comme hygiène de décision (Jobs : exercice du miroir)
- [[obsolescence-logicielle]] — Fonctionnalité payée rendue inaccessible par changement matériel/politique
- [[open-source-etat]] — Open source comme posture stratégique (État, infra, IA)
- [[optimisation-couts-ia]] — Réduction coûts LLM : compression tokens, caching, routing
- [[perseverance]] — La moitié de ce qui sépare entrepreneurs qui réussissent/échouent (Jobs)
- [[rentabilite-ia]] — Tension entre adoption massive et absence de profitabilité
- [[securite-infrastructure]] — Veille CVE, CVSS, KEV, CERT-FR
- [[securite-proof-of-work]] — Sécurité = course aux tokens, proof-of-work appliqué à la cybersécurité
- [[self-hosting]] — Héberger soi-même vs SaaS
- [[serverless-zero-cost]] — Pattern Lambda free tier + Function URL à coût zéro
- [[souverainete-numerique]] — Contrôle des données et infrastructures (État + individu)
- [[spec-driven-development]] — La doc est le design (Royce, redécouvert via les LLM)
- [[super-app-ia]] — Plateforme unifiée regroupant plusieurs capacités IA
- [[sycophantie-llm]] — LLM qui flattent plutôt que contredire l'utilisateur
- [[techno-optimisme]] — La technologie comme moteur principal du progrès humain
- [[vocation-professionnelle]] — Aimer son travail comme prérequis du grand travail (Jobs)

## Syntheses

- [[bilan-ingest-openai-122md]] — Bilan du premier ingest : 14 pages créées, questions ouvertes sur la rentabilité IA
- [[bilan-ingest-apprendre-a-coder]] — Nuance émergente : l'IA démocratise pour ceux qui maîtrisent les bases
- [[reponse-post-280-dollars-anthropic]] — Post LinkedIn : critique des unit economics IA, plaidoyer open source vs dépendance aux labs
- [[bilan-batch-ingest-avril-2026]] — Bilan du batch de 12 sources : 4 clusters thématiques émergents (souveraineté, unit economics, critique LLM, infra minimaliste)
- [[post-80-20-experience-ia]] — Post LinkedIn : 80 % des compétences accessibles via IA, les 20 % d'expérience ne se compressent pas
